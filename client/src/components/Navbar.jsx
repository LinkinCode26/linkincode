...
// Agregar contacto como sección observable separada
const ALL_SECTIONS = [
  ...NAV_ITEMS,
  { key: 'contacto', href: '#contacto' }, // Solo para observación, no para el menú
];
...
export function Navbar() {
  ...
  // Scroll-Spy con Intersection Observer actualizado
  useEffect(() => {
    const sectionIds = ALL_SECTIONS.map((item) => item.href.replace('#', ''));
    ...
    <>
      ...
      <header className="fixed top-0 left-0 w-full z-40 bg-[rgb(var(--nav-bg))]/80 backdrop-blur-xl border-b border-line transition-colors duration-300">
        <div className="container mx-auto max-w-7xl px-6 h-20 flex items-center justify-between">
          ...
          {/* Enlaces Desktop - sin cambios para NAV_ITEMS */}
          ...
          {/* Acciones Desktop - Botón de contacto con estado activo */}
            ...
            <Button
              ...
              className={`font-bold tracking-widest text-xs uppercase px-6 py-2.5 rounded-xl shadow-lg transition-all ${
                activeSection === 'contacto' 
                  ...
                  : 'shadow-brand/20 hover:shadow-brand/30 hover:scale-105'
              }`}
            >
      ...
      >
        ...
        <div className="pt-6 border-t border-line flex flex-col gap-5">
          ...
          <Button
            ...
            onClick={closeDrawer}
            variant="brand"
            className={`w-full justify-center text-xs font-bold uppercase tracking-widest py-3 rounded-xl shadow-lg transition-all ${
              activeSection === 'contacto' 
                ? 'shadow-brand/40 ring-2 ring-brand/50' 
                : 'shadow-brand/20'
            }`}
          >
            {t('nav.contacto') || 'CONTACTO'}
