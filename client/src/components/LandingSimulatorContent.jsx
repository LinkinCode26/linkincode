...
const BRAND_COLOR_OPTIONS = [
  { id: 'brand', dotClass: 'bg-brand', textClass: 'text-brand', buttonClass: 'bg-brand hover:bg-brand/90' },
  { id: 'accent', dotClass: 'bg-accent', textClass: 'text-accent', buttonClass: 'bg-accent hover:bg-accent/90' },
  { id: 'amber', dotClass: 'bg-amber-500', textClass: 'text-amber-500', buttonClass: 'bg-amber-500 hover:bg-amber-500/90' },
  { id: 'pink', dotClass: 'bg-pink-500', textClass: 'text-pink-500', buttonClass: 'bg-pink-500 hover:bg-pink-500/90' },
  { id: 'emerald', dotClass: 'bg-emerald-500', textClass: 'text-emerald-500', buttonClass: 'bg-emerald-500 hover:bg-emerald-500/90' },
];
...
export function LandingSimulatorContent() {
  ...
  const selectedBrandColor =
    BRAND_COLOR_OPTIONS.find((option) => option.id === brandColorId) ?? BRAND_COLOR_OPTIONS[0];
  const selectedBgColor =
    BACKGROUND_COLOR_OPTIONS.find((option) => option.id === bgColorId) ?? BACKGROUND_COLOR_OPTIONS[0];

  const previewTitle = title.trim() || t('solutions.simulator.simulateHeadlineDefault');
  const previewSubtitle = subtitle.trim() || t('solutions.simulator.simulateDescription');
  ...
  return (
    <div className="flex flex-col gap-6 py-2">
          ...
          {BRAND_COLOR_OPTIONS.map((option) => (
            <button
              ...
              className={`w-8 h-8 rounded-full ${option.dotClass} border-2 transition-transform ${
                brandColorId === option.id ? 'scale-110 border-white shadow-lg' : 'border-transparent opacity-60'
              }`}
            />
          ))}
        </div>
        ...
        <div className="flex gap-3">
          {BACKGROUND_COLOR_OPTIONS.map((option) => (
            <button
              ...
              aria-label={option.id}
              className={`w-8 h-8 rounded-full ${option.dotClass} border-2 transition-transform ${
                bgColorId === option.id ? 'scale-110 border-white shadow-lg' : 'border-line opacity-70'
              }`}
            />
          ))}
        </div>
      ...
      <div
        className={`mt-4 p-6 rounded-xl ${selectedBgColor.bgClass} border border-line text-center flex flex-col items-center gap-3 transition-colors duration-300`}
      >
        <p className="text-xs text-mute">{t('solutions.simulator.previewLabel')}</p>
        <h4 className={`font-display font-bold text-2xl ${selectedBrandColor.textClass}`}>
          {previewTitle}
        </h4>
